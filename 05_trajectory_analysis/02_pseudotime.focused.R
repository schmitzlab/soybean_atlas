...
# load arguments
arg <- commandArgs(T)
if(length(arg)!= 6){stop("pseudotime.R <sparse> <meta> <svd> <prefix> <config> <threads>")}
sparse <- as.character(arg[1])
meta <- as.character(arg[2])
svd <- as.character(arg[3])
prefix <- as.character(arg[4])
config <- as.character(arg[5])
threads <- as.numeric(arg[6])
...
loadData   <- function(sm, ma, rd, doL2=0, traj, column="top_tissue_cluster"){
...
plotPT     <- function(meta, subsetCluster=NULL, t.id="trajectory", umap1="umapsub_1", umap2="umapsub_2",
    ...
    meta$celltypeID <- meta$pt_celltype
    ...
    test.2 <- test.3
    #print(head(test.2))
    plot(test.2[,c(umap1)], test.2[,c(umap2)], col=as.character(test.2$emb_color), pch=16,
    ...
    legend("topright", legend=sort(unique(as.character(test.2$celltypeID))), 
           col=as.character(test.2$emb_color)[factor(sort(unique(as.character(test.2$celltypeID))),levels=sort(unique(as.character(test.2$celltypeID))))])
...
###################################################################################################
obj <- loadData(sparse, meta, svd, doL2=1, traj, column=column)
...
plotPT(out, subsetCluster=NULL, smoother=5, t.id="trajectory", umap1=umap1, umap2=umap2)
dev.off()
